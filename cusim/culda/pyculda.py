...
class CuLDA:
    def __init__(self, opt=None):
        ...
        self.alpha, self.beta, self.grad_alpha, self.new_beta = None, None, None, None
    ...
    def init_model(self):
        ...
        self.alpha = np.random.uniform(size=(self.opt.num_topics,)).astype(np.float32)
        self.beta = np.random.uniform(size=(self.num_words, self.opt.num_topics)).astype(np.float32)
        self.beta /= np.sum(self.beta, axis=0)[None, :]
        self.logger.info("alpha %s, beta %s initialized", self.alpha.shape, self.beta.shape)

        # zero initialize grad alpha and new beta
        block_cnt = self.obj.get_block_cnt()
        self.grad_alpha = np.zeros(shape=(block_cnt, self.opt.num_topics), dtype=np.float32)
        self.new_beta = np.zeros(shape=self.beta.shape, dtype=np.float32)
        self.logger.info("grad alpha %s, new beta %s initialized", self.grad_alpha.shape, self.new_beta.shape)
        ...
        h5f.create_dataset("gamma", shape=(self.num_docs, self.opt.num_topics), dtype=np.float32)
        h5f.close()
    ...
    def _train_e_step(self, h5f, gamma_h5f, epoch):
        ...
        while True:
            ...
            # call cuda kernel
            train_loss, vali_loss = self.obj.feed_data(
                cols,
            ...
            # update progress bar
            pbar.update(end, values=[("train_loss", train_loss), ("vali_loss", vali_loss)])
            offset = next_offset
    ...
    def save_h5_model(self, filepath, chunk_size=10000):
        ...
        gamma = h5f.create_dataset("gamma", dtype=np.float32, shape=(self.num_docs, self.opt.num_topics))
