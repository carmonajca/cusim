...
class CuW2V:
    def __init__(self, opt=None):
        ...
        assert (
            self.opt.block_dim <= WARP_SIZE**2 and self.opt.block_dim % WARP_SIZE == 0
        ...
        self.words, self.word_count, self.num_words, self.num_docs = None, None, None, None
    ...
    def preprocess_data(self):
        ...
        iou.convert_stream_to_h5(self.opt.data_path, self.opt.word_min_count, self.opt.processed_data_dir)
    ...
    def init_model(self):
        ...
        with open(count_path, "rb") as fin:
            self.word_count = np.array([int(line.strip()) for line in fin], dtype=np.int64)
        ...
        self.logger.info("number of words: %d, docs: %d", self.num_words, self.num_docs)

        # normalize word count
        word_count = np.power(self.word_count, self.opt.count_power, dtype=np.float64)
        ...
        self.emb_in = np.random.normal(loc=0, scale=scale, size=(self.num_words, self.opt.num_dims)).astype(np.float32)
        out_words = self.num_words if self.opt.neg else self.num_words - 1
        self.emb_out = np.random.normal(loc=0, scale=scale, size=(out_words, self.opt.num_dims)).astype(np.float32)
        self.logger.info("emb_in %s, emb_out %s initialized", self.emb_in.shape, self.emb_out.shape)
    ...
    def _train_epoch(self, h5f):
        ...
        while True:
            ...
            # call cuda kernel
            _loss_nume, _loss_deno = self.obj.feed_data(cols, indptr.astype(np.int32))
    ...
    def save_h5_model(self, filename):
        ...
        h5f.create_dataset("keys", data=np.array([word.encode("utf") for word in self.words]))
        h5f.close()

    def save_word2vec_format(self, filename, binary=False, prefix=""):
        self.logger.info("save word2vec format model to %s, " "binary: %s, prefix: '%s'", filename, binary, prefix)
        ...
        with open(filename, "wb") as fout:
            ...
            for idx, word in enumerate(self.words):
                ...
                if binary:
                    ...
                    fout.write(f"{prefix}{word} " f"{' '.join(repr(val) for val in vec)}\n".encode("utf8"))

    def load_word2vec_format(self, filename, binary=False, symmetry=False, no_header=False):
        ...
        with open(filename, "rb") as fin:
            ...
            for line in fin:
                if binary:
                    ...
                    vec = np.array([float(val) for val in line_vec[1:]], dtype=np.float32)
                    vector_dict[key] = vec
