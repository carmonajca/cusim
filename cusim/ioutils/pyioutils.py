...
class IoUtils:
    ...
    def load_stream_vocab(self, filepath, min_count, keys_path, count_path):
        full_num_lines = self.obj.load_stream_file(filepath)
        pbar = aux.Progbar(full_num_lines, unit_name="line", stateful_metrics=["word_count"])
        processed = 0
        while True:
            read_lines, word_count = self.obj.read_stream_for_vocab(self.opt.chunk_lines, self.opt.num_threads)
    ...
    def convert_stream_to_h5(self, filepath, min_count, out_dir, chunk_indices=10000, seed=777):
        np.random.seed(seed)
        os.makedirs(out_dir, exist_ok=True)
        keys_path = pjoin(out_dir, "keys.txt")
        count_path = pjoin(out_dir, "count.txt")
        token_path = pjoin(out_dir, "token.h5")
        self.logger.info("save key, count, token to %s, %s, %s", keys_path, count_path, token_path)
        ...
        rows = h5f.create_dataset(
            "rows", shape=(chunk_indices,), maxshape=(None,), dtype=np.int64, chunks=(chunk_indices,)
        )
        cols = h5f.create_dataset(
            "cols", shape=(chunk_indices,), maxshape=(None,), dtype=np.int32, chunks=(chunk_indices,)
        )
        vali = h5f.create_dataset(
            "vali", shape=(chunk_indices,), maxshape=(None,), dtype=np.float32, chunks=(chunk_indices,)
        )
        indptr = h5f.create_dataset("indptr", shape=(full_num_lines + 1,), dtype=np.int64, chunks=True)
        processed, offset = 1, 0
        indptr[0] = 0
        while True:
            read_lines, data_size = self.obj.tokenize_stream(self.opt.chunk_lines, self.opt.num_threads)
            ...
            rows[offset : offset + data_size] = _rows.astype(np.int64) + (processed - 1)
            cols.resize((offset + data_size,))
            cols[offset : offset + data_size] = _cols
            vali.resize((offset + data_size,))
            vali[offset : offset + data_size] = np.random.uniform(size=(data_size,)).astype(np.float32)
            indptr[processed : processed + read_lines] = _indptr.astype(np.int64) + offset
            offset += data_size
            processed += read_lines
    ...
    def convert_bow_to_h5(self, filepath, h5_path):
        ...
        num_docs, num_words, num_lines = self.obj.read_bag_of_words_header(filepath)
        self.logger.info("number of docs: %d, words: %d, nnz: %d", num_docs, num_words, num_lines)
        h5f = h5py.File(h5_path, "w")
        rows = h5f.create_dataset("rows", dtype=np.int64, shape=(num_lines,), chunks=True)
        cols = h5f.create_dataset("cols", dtype=np.int32, shape=(num_lines,), chunks=True)
        counts = h5f.create_dataset("counts", dtype=np.float32, shape=(num_lines,), chunks=True)
        vali = h5f.create_dataset("vali", dtype=np.float32, shape=(num_lines,), chunks=True)
        indptr = h5f.create_dataset("indptr", dtype=np.int64, shape=(num_docs + 1,), chunks=True)
        indptr[0] = 0
        ...
        while processed < num_lines:
            ...
            vali[processed : processed + read_lines] = np.random.uniform(size=(read_lines,)).astype(np.float32)
            ...
            if _indptr:
                indptr[indptr_offset : indptr_offset + len(_indptr)] = np.array(_indptr, dtype=np.int64)
